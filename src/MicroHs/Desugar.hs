...
type Arm = ([EPat], Exp -> Exp, Exp -> Exp)  -- Patterns, and a function that expects the default (which might be ignored).
type Matrix = [Arm]
...
dsMatrix dflt iis@(i:is) aarms@(aarm : aarms') =
  case leftMost aarm of
    EVar _ -> do
      ...
      let (vars, nvars) = span (isPVar . leftMost) aarms
          vars' = map (subs . unAt i) vars
          subs (EVar x : ps, sub, rhs) = (ps, substAlpha x i . sub, rhs)
          subs _ = impossible
    ...
    EViewPat e _ -> do
      ...
      letBind (dsMatrix dflt iis nviews) $ \ drest ->
        letBind (return $ App (sub $ dsExpr e) i) $ \ vi -> do
  ...
  where
    ...
    isPView _ _ = False
    unAt ii (EAt x p : ps, sub, rhs) = unAt i (p:ps, substAlpha x ii . sub, rhs)
    unAt _ arm = arm
