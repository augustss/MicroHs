...
#MHSGMPCCFLAGS=-I/opt/homebrew/include -DWANT_GMP=1
#MHSGMPLIBS=-L/opt/homebrew/lib -lgmp
#MHSGMP=-ilib/gmp
#MCABALGMP=-fgmp
#
# installation prefix
...
GHCPKGS= -package mtl -package pretty -package haskeline -package process -package time -package ghc-prim -package containers -package deepseq -package directory -package text -package bytestring -package filepath -package array -package base
...
bin/mhseval:	src/runtime/*.c src/runtime/config*.h
	@mkdir -p bin
	$(CCEVAL) src/runtime/comb.c $(CCLIBS) -o bin/mhseval
	size bin/mhseval

bin/mhsevalgdb:	src/runtime/*.c src/runtime/config*.h
	@mkdir -p bin
	$(CC) $(CCWARNS) $(MHSGMPCCFLAGS) -g src/runtime/eval-$(CONF).c src/runtime/comb.c $(CCLIBS) -o bin/mhsevalgdb

bin/mhsevalsane:	src/runtime/*.c src/runtime/config*.h
	@mkdir -p bin
	$(CCEVAL) $(CCSANITIZE) src/runtime/comb.c $(CCLIBS) -o bin/mhsevalsane
